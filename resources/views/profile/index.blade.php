    ...
    <div class="row">
                ...
                @foreach($statuses as $status)
                    <div class="media">
                        ...
                        <div class="media-body">
                            <h4 class="media-heading"><a href="{{url('user/profile/index', ['username' => $status->user->username])}}">
                            ...
                            @foreach($status->replies as $reply)
                                <div class="media">
                                    ...
                                    <div class="media-body">
                                        <h5 class="media-body"><a href="{{url('user/profile/index', ['username' => $status->user->username])}}">{{$reply->user->getNameOrUsername()}}</a></h5>
                                        <p>{{$status->body}}</p>
                                        <ul class="list-inline">
                            ...
                            {{--@if($authUserIsFriend || Auth::user()->id === $status->user->id)--}}
                                <form role="form" action="{{url('user/status/reply', ['statusId' => $status->id])}}" method="post">
        ...
        <div class="col-lg-4 col-lg-offset-3">
            ...
            @elseif (Auth::user()->hasFriendRequestReceived($user))
                <a href="{{url('user/friend/accept', ['username' => $user->username])}}" class="btn btn-primary">Accept friend request</a>
            @elseif(Auth::user()->isFriendsWith($user))
                <p>You and{{$user->getNameOrUserName()}} are friends</p>

                <form action="{{url('user/friend/delete', ['username' => $user->username])}}" method="post">
                    <input type="submit" value="Delete friend" class="btn btn-primary">
                    <input type="hidden" name="_token" value="{{csrf_token()}}">
                </form>

            @elseif(Auth::user()->id !== $user->id)
                <a href="{{url('user/friend/add', ['username' => $user->username])}}" class="btn btn-primary">Add as friend</a>
            @endif
