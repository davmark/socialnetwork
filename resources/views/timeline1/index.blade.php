...
@section('content')
    <div class="row">
        <div class="col-lg-6">
            <form role="form" action="{{url('user/status/post')}}" method="post">
                <div class="form-group{{$errors->has('status') ? 'has-error' : ''}}">
                    <textarea placeholder="What's up {{$user->getFirstNameOrUsername()}}" name="status"
                ...
                @foreach($statuses as $status)
                    <div class="media">
                        ...
                        <div class="media-body">
                            <h4 class="media-heading"><a href="{{url('profile/index',
                            ...
                            @foreach($status->replies as $reply)
                                <div class="media">
                                    <a class="pull-left" href="{{url('profile/index', ['username' => $reply->user->username])}}">
                                        <img class="media-object" alt="{{$reply->user->getNameOrUsername()}}"
                                             src="{{$reply->user->getAvatarUrl()}}">
                                    </a>
                                    <div class="media-body">
                                        <h5 class="media-body"><a href="{{url('profile/index', ['username' => $reply->user->username])}}">{{$reply->user->getNameOrUsername()}}</a></h5>
                                        <p>{{$reply->body}}</p>
                                        <ul class="list-inline">
                            ...
                            <form role="form" action="{{url('user/status/reply', ['statusId' => $status->id])}}" method="post">
                                <div class="form-group{{$errors->has("reply") ? ' has-error': ''}}">
                                    <textarea name="reply" class="form-control" rows="2" placeholder="Reply to this status"></textarea>
                                    @if($errors->has("reply"))
