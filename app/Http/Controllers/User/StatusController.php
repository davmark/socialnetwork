...
class StatusController extends UserBaseController
{
    ...
    public function postCreateReply( StatusService $statusService )
    {
        ...
        if($status)
            return back()->with('success','Successfuly created');
        return back()->with('success','Can not create reply');
    }
     ...
     * @return view
     */
    public function deleteDeleteReply( StatusService $statusService, $id)
    {
        $statusService->setModel('statusReply');
        
        if($statusService->delete($id, $user))
            return back()->with('success','Successfuly deleted');
        return back()->with('success','Can not delete reply');
    }
}